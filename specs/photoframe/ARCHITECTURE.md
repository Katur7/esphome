...
ESPHome config for **photoframe** — a battery-powered 7.3" color e-paper photo frame. Fetches a pre-rendered 800×480 PNG over HTTP and draws it. The device does no photo selection: whatever serves the URL owns picking, cropping, and palette-mapping.
...
**The U.FL antenna must be physically attached.** Without it the WiFi scan returns zero networks — not a missing SSID, no APs at all — which reads like a credentials problem but isn't.

## Open Items

- **Image pipeline not built.** `photo_url` points at a placeholder path, and the whole block is commented out in `display.package.yaml` while the bring-up test pattern is in place. Something still has to produce an 800×480 PNG mapped to the 6-color palette; naive RGB PNGs will dither poorly.
- **~1–2 px at the extreme edge** sits under the panel bezel. Keep content inset.
- **Deep sleep + battery monitoring** (task 02). A full refresh measures **~31 s** on this panel and is by far the biggest power draw, so refresh cadence and battery life are tightly coupled. einkframe's `voltage_to_battery_percent` and slot-based wake schedule are the obvious things to reuse.
