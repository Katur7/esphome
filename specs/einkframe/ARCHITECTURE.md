...
- **einkframe_utils** — Pure functions (no ESPHome deps). Currently: `minutes_to_next_slot(hour, minute)`, `voltage_to_battery_percent(voltage)`, `weather_icon(state)` (HA weather state → MDI codepoint), `ui_icon(key)` (internal UI key → MDI codepoint), `wrap_text_pure(text, max_width, measurer)` (font-agnostic wrapping), `split_translations(s)` and `cap_translations(items, max)` (parse + cap the comma/middle-dot translation list), `fit_headword_size_index(text, ladder_size, max_width, measurer)` (pick the largest discrete font size that fits), `weather_label(state)` (HA weather state → Swedish word), and `pos_long(pos)` (extract spelled-out word class from `"no. (nafnorð)"`). Add further extractable pure logic here.
- **renderer_utils** — Display orchestration for the 07a "Orð dagsins" layout. Exports three panel draw functions (`draw_folio_header`, `draw_entry`, `draw_strip`) and their per-panel state structs (`FolioHeaderState`, `EntryState`, `StripState`, plus `ExamplePair` and `StripCell`). Each function takes a `*Fonts` bundle (`FolioFonts`, `EntryFonts`, `StripFonts`) rather than a flat list of `Font*` args. Layout constants, the `draw_peg` / `draw_wrapped` / `draw_dotted_translations` helpers, and the internal hero/examples-column split live `static` in the `.cpp`. All coords are anchored inside the panel's usable print area. Couples to `esphome::display::Display` and `esphome::font::Font` — not unit-testable in the doctest harness; verified by hardware.
